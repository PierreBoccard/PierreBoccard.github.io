...
(function () {
    ...
    function applyProgress(state, progress, opts) {
        ...
        /* Continuous "visit" index. v is the current void being revealed. */
        var visitsF = progress * N_VOIDS;
        var v = Math.min(N_VOIDS - 1, Math.floor(visitsF));
        ...
        for (var i = 0; i < N_VOIDS; i++) {
            var V = VOIDS[i];
            var revealT;       /* 0..1 for circle / connector reveal */
            if (i < v) revealT = 1;
            else if (i === v) revealT = withinT;
            else revealT = 0;
            ...
            /* Connector path: curved bezier from void center to the top of
               the bin bar. We draw the full path and animate
               stroke-dashoffset for the "drawing in" effect. */
            var binIdx = V.bin;
            var binCenterX = barX(binIdx) + BAR_W / 2;
            /* Target y is the eventual top of the bar AFTER this void lands */
            var willCount = counts[binIdx] + 1;
            var targetY   = barTopY(willCount);
            /* mid control point: out to the right of the void, then sweeping
               up to the bar top */
            ...
            /* Use stroke-dasharray + offset to draw progressively */
            var path = state.connectors[i];
            var totalLen = path.getTotalLength ? path.getTotalLength() : 600;
            path.style.strokeDasharray = totalLen;
            path.style.strokeDashoffset = (totalLen * (1 - revealT)).toFixed(1);
            path.style.opacity = (revealT > 0.05 ? 1 : 0);

            /* Bar count contribution: 0 if not revealed yet, fractional during
               reveal so the bar grows smoothly as the line completes. */
            if (i < v) counts[binIdx] += 1;
            else if (i === v) counts[binIdx] += withinT;
        }
        ...
        /* ---- HUD ----------------------------------------------------- */
        var totalRevealed = Math.min(N_VOIDS, v + (withinT > 0 ? 1 : 0));
        if (hudCount) hudCount.textContent = totalRevealed + ' / ' + N_VOIDS;
        if (hudRadius) {
            var curV = VOIDS[v];
            hudRadius.textContent = 'R ~ ' + (curV.r * 0.6).toFixed(0) + ' h^-1 Mpc';
        }
